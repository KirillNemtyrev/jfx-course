...
public class UpdateNameHandler implements Callback<ResponseBody> {
    ...
    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
        if (!response.isSuccessful()) {
            messageDraw.setMessage("Ошибка", "Не удалось обновить имя..");
            messageDraw.error();
            messageDraw.show();
            return;
        }
        ...
        messageDraw.setMessage("Успешно", "Вы успешно изменили имя!");
        messageDraw.info();
        messageDraw.show();
